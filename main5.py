...
class StoryVideoGeneratorV5:
    ...
    def __init__(
        ...
        logger.info("Initialized StoryVideoGeneratorV5 for project: %s", project_name)
        ...
        if scene_prompt:
            logger.info("Using custom scene prompt: %s", scene_prompt)
    ...
    def run(self, audio_path: str) -> str:
        ...
        try:
            ...
            logger.info("=" * 60)
            logger.info("Starting main5 pipeline (Whisper + LLM scenes)")
            ...
            logger.info("\n" + "=" * 60)
            logger.info("✓ main5 video generation completed successfully!")
...
def main():
    """Main entry point for main5 pipeline"""
    parser = argparse.ArgumentParser(
        description=(
            "Vietnamese Story Audio to Video Generator (main5) - "
            "Local Whisper + LLM scene generation with custom prompt"
        ),
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python main5.py --audio story.mp3 --project my_story
  python main5.py --audio tnk.mp3 --project tnk --whisper-model small --max-scene-duration 8
  python main5.py --audio input.mp3 --project demo \\
      --api-key YOUR_API_KEY --whisper-model base --language vi \\
      --scene-prompt "A children's story about a brave little bird, watercolor style"
        """,
    )
    ...
    parser.add_argument(
        "--scene-prompt",
        default="",
        help="A short prompt to guide the scene generation, e.g., 'A children's story, watercolor style'",
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
    ...
    try:
        generator = StoryVideoGeneratorV5(
        ...
        print(f"\n✓ Success! main5 video saved to: {video_path}")
    except Exception as e:
        logger.error("Fatal error in main5: %s", str(e))
        sys.exit(1)
